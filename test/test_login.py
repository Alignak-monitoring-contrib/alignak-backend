...
class Test_0_LoginCreation(unittest2.TestCase):
    ...
    def test_0(self):
        ...
        print("")
        print("start backend")
        self.p = subprocess.Popen(['uwsgi', '--plugin', 'python', '-w', 'alignakbackend:app', '--socket', '0.0.0.0:5000',
                                   '--protocol=http', '--enable-threads', '--pidfile',
                                   '/tmp/uwsgi.pid'])
        time.sleep(3)
...
class Test_1_Login(unittest2.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.p = subprocess.Popen(['uwsgi', '--plugin', 'python', '-w', 'alignakbackend:app', '--socket', '0.0.0.0:5000',
                                  '--protocol=http', '--enable-threads', '--pidfile',
                                  '/tmp/uwsgi.pid'])
        time.sleep(3)
