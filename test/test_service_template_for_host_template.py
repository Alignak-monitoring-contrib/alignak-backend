...
class TestTemplatesUpdate(unittest2.TestCase):
    ...
    def test_add_service_on_host_template(self):
        """Test we can't add a service (not template) on a host template
        ...
        # try create a service on a host_template
        item = {}
        item['name'] = 'test_service'
        item['_realm'] = self.realm_all_id
        item['check_period'] = self.tp_24_7_id
        # Get host
    ...
    def test_service_template_update_is_template_false(self):
        """Test the service template update - update the _is_template property to False
        It will not be possible to have a service not template on host template, so will have an
        error when try modify the _is_template field from True to False

        :return: None
        """
        # This property is True because it is for a template...
        # Setting to False breaks the templates and inherited services!
        data_to_update = {
            '_is_template': False
        }
        ...
        # Update the service element
        params = {'where': json.dumps({'name': 'linux_nrpe_version', '_is_template': True})}
        response = requests.get(self.endpoint + '/service', params=params, auth=self.auth)
        items = response.json()
        assert len(items['_items']) == 1
        services = items['_items']
        ...
        headers = {'Content-Type': 'application/json', 'If-Match': my_template['_etag']}
        response = requests.patch(self.endpoint + '/service/%s' % my_template['_id'],
                                  json=data_to_update, headers=headers, auth=self.auth)
        assert response.status_code == 412
