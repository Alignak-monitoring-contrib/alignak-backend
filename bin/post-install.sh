...
# --------------------------------------------------------------------------------
# This script is doing the post-install processing for Alignak Backend application:
...
then
   ...
   then
      ...
      echo "You can then run: sudo $PREFIX/share/alignak-backend/post-install.sh"
      exit 1
   fi
else
   ...
   if [ "$pyver" = "2.7" ]; then
      ...
      then
         ...
         echo "You can then run: $PREFIX/share/alignak-backend/post-install.sh"
         exit 1
      fi
   else
...
cp $PREFIX/share/alignak-backend/log-rotate-alignak-backend /etc/logrotate.d/alignak-backend
cp $PREFIX/share/alignak-backend/log-rotate-uwsgi /etc/logrotate.d/alignak-backend-uwsgi
echo "Installed."
